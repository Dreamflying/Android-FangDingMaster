...
/**
 * @author liyuan
 * @Description 首页导航Activity
 * @Email 1522651962@qq.com
 * @date 2014年8月6日下午5:21:52
 */
public abstract class IntroActivity extends Activity implements OnClickListener,
    ...
    private boolean isAfterUpdate() {
        try {
            int currentVersion = this.getPackageManager().getPackageInfo(this.getPackageName(), 0).versionCode;
            int oldVersion = this.getSharedPreferences(AK_PREF, MODE_PRIVATE).getInt(INTRO_KEY, -1);
    ...
    public void onPageScrolled(int pos, float offset, int pixels) {
        if ((pos + 1) == mIndicator.size() && offset * 100000 < 1 && pixels == 0) {
            mScroller.setMoveDisable(true);
        } else {
            mScroller.setMoveDisable(false);
        }
    }
