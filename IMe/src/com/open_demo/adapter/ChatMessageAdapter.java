...
public class ChatMessageAdapter extends BaseAdapter {
	...
	public ChatMessageAdapter(ChatPage activity, List<GotyeMessage> messageList,String username) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (message.getSender().name.equals(SharePreferenceUtils.getIntance(chatPage).getCurrentUserName())) {
			ImageLoader.getInstance().displayImage(
					SharePreferenceUtils.getIntance(chatPage).getHeadImgUserName(),
					holder.head_iv,
			...
			ImageLoader.getInstance().displayImage(
					SQLDatebaseUtils.getInstance(chatPage).getHeadImageByUsername(username),
					holder.head_iv,
					InitDisplayImageOptions.getInstance().getHeadOptions(false,
							false, 0),
					InitDisplayImageOptions.getInstance()
		...
		holder.head_iv.setTag(position);
		holder.head_iv.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				if (message.getSender().name.equals(SharePreferenceUtils.getIntance(chatPage).getCurrentUserName())) {
					...
					int currentPosition = (Integer) arg0.getTag();
					Intent myinfoIntent = new Intent(chatPage, MyselfInfoActivity.class);
					myinfoIntent.putExtra("getuser",username);
					myinfoIntent.putExtra("nickname",SQLDatebaseUtils.getInstance(chatPage).getNickNameByUsername(username));
					myinfoIntent.putExtra("headimage", SQLDatebaseUtils.getInstance(chatPage).getHeadImageByUsername(username));
					myinfoIntent.putExtra("ido", SQLDatebaseUtils.getInstance(chatPage).getIdoByUsername(username));
					myinfoIntent.putExtra("from", true);
					chatPage.startActivity(myinfoIntent);		
				}
	...
	private void handleImageMessage(final GotyeMessage message,
		...
		holder.iv.setImageResource(R.drawable.logo_s);
	...
	private void handleVoiceMessage(final GotyeMessage message,
		...
		if (isPlaying) {
			...
			if (getDirect(message) == MESSAGE_DIRECT_RECEIVE) {
				holder.iv.setImageResource(R.drawable.gotye_anim_voice_3);
			} else {
				holder.iv.setImageResource(R.drawable.gotye_anim_voice_right_3);
			}
		}
		if (getDirect(message) == MESSAGE_DIRECT_RECEIVE) {
