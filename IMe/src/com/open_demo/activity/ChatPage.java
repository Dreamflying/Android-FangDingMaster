...
import com.utils.widght.VoicePop;
...
public class ChatPage extends BaseActivity implements OnClickListener {
	...
	public void initView() {
		...
		if (user != null) {
			chatType = 0;
			if (SQLDatebaseUtils.getInstance(ChatPage.this)
					.getNickNameByUsername(user.name) != null) {

				if (!SQLDatebaseUtils.getInstance(ChatPage.this)
						.getNickNameByUsername(user.name).equals("null")) {
					title.setText("和 "
							+ SQLDatebaseUtils.getInstance(ChatPage.this)
									.getNickNameByUsername(user.name) + " 聊天");
				} else {
					title.setText("和 " + "陌生人" + " 聊天");
				}
			} else {
		...
		textMessage.setOnEditorActionListener(new OnEditorActionListener() {
			...
			public boolean onEditorAction(TextView arg0, int arg1, KeyEvent arg2) {
				...
				sendTextMessage(text
						+ "@"
						+ SharePreferenceUtils.getIntance(ChatPage.this)
								.getCurrentNickName()
						+ "@"
						+ SharePreferenceUtils.getIntance(ChatPage.this)
								.getHeadImgUserName()
		...
		pressToVoice.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_DOWN:
					VoicePop.getIntance().showVoice(ChatPage.this, v);
		...
		pullListView.requestFocusFromTouch();
	...
	private void sendTextMessage(String text) {
		if (!TextUtils.isEmpty(text)) {
			GotyeMessage toSend;
			if (chatType == 0) {
				GotyeUser user = new GotyeUser(username);
	...
	private void setListViewInfo() {
		 ...
		 * 
		 * @Override public void onRefresh() { if (chatType == 1) {
		 * api.getLocalMessages(room, true); } else { List<GotyeMessage> list =
		 * null;
		 * 
		 * if (chatType == 0) { list = api.getLocalMessages(user, true); } else
		 * if (chatType == 2) { list = api.getLocalMessages(group, true); } if
		 * (list != null) { for(GotyeMessage msg:list){
		 * api.downloadMessage(msg); } adapter.refreshData(list); } else {
		 * ToastUtil.show(ChatPage.this, "没有更多历史消息"); } }
		 * adapter.notifyDataSetChanged(); pullListView.onRefreshComplete(); }
		 * });
		 */
	...
	public void onGetHistoryMessageList(int code, List<GotyeMessage> list) {
		if (chatType == 1) {
			List<GotyeMessage> listmessages = api.getLocalMessages(room, false);
	...
	public void onStopTalk(int code, GotyeMessage message, boolean isVoiceReal) {
		if (isVoiceReal) {
			...
			message.putExtraData(StringUtils
					.stringToByteArray(SharePreferenceUtils.getIntance(
							ChatPage.this).getCurrentNickName()
							+ "@"
							+ SharePreferenceUtils.getIntance(ChatPage.this)
									.getHeadImgUserName()
							+ "@"
